...
int main(int argc, char **argv)
{
    ...
    while (is_mod_playing() == TRUE)
        {
        //acquire_screen();
        textprintf_ex (screen, font, 0,36, font_color, 0, "Tempo : %3d  Bpm : %3d  Speed : %3d%%  Pitch : %3d%% ", mi.tempo, mi.bpm, mi.speed_ratio, mi.pitch_ratio);
        textprintf_ex (screen, font, 0,48, font_color, 0, "Global volume : %2d  User volume : %2d ", mi.global_volume, get_mod_volume());
        textprintf_ex (screen, font, 0,70, font_color, 0, "%03d-%02d-%02d    ", mi.trk, mi.pos, mi.tick < 0 ? 0 : mi.tick);
        ...
        for (index=start_chn; index<end_chn; index++)
            ...
            if ( (voice_get_position(voice_table[index]) >= 0) && (ci[index].volume >= 1) && (ci[index].volenv.v >= 1) && (voice_get_frequency(voice_table[index]) > 0) && (mi.global_volume > 0) )
                textprintf_ex (screen, font, 0,82+(index-start_chn)*bitmap_height, font_color, 0, "%2d: %3d %2d %6dHz %3d ", index+1, ci[index].sample+1, ci[index].volume,  voice_get_frequency(voice_table[index]), ci[index].pan);
                //textprintf_ex (screen, font, 0,82+(index-start_chn)*bitmap_height, font_color, 0, "%2d: %3d %2d %6dHz %3d %d %d", index+1, ci[index].sample+1, ci[index].volume,  voice_get_frequency(voice_table[index]), ci[index].pan, ci[index].volenv.v, ci[index].volenv.p);
            else
                textprintf_ex (screen, font, 0,82+(index-start_chn)*bitmap_height, font_color, 0, "%2d: %3s %2s %6sHz %3s  ", index+1, " --", "--",  " -----", "---"); 
                //textprintf_ex (screen, font, 0,82+(index-start_chn)*bitmap_height, font_color, 0, "%2d: %3d %2s %6sHz %3s ", index+1, ci[index].sample+1, "--",  " -----", "---"); 
...
{
    clear(screen);
    textprintf_ex (screen, font, 0,0, font_color, 0, "Song name   : %s", the_mod->name);
    textprintf_ex (screen, font, 0,12, font_color, 0, "No Channels : %2d  Period Type : %s  No Inst : %2d ", the_mod->no_chn, (the_mod->flag & LINEAR_MODE) ? "Linear" : "Amiga", the_mod->no_instrument);
    textprintf_ex (screen, font, 0,24, font_color, 0, "No Tracks   : %2d  No Patterns : %2d  No Sample : %2d ", the_mod->no_trk, the_mod->no_pat, the_mod->no_sample);
}
